            	...
            	<div class="login-container">
					<c:if test="${ empty loginMember }">
					  <div>
                  	  	...
                  	  	<button type="button" id="si" onclick="location.href='${ path }/member/signup'" class="btn btn-outline-secondary">회원가입</button>
                   	 	<button type="button" id="so" onclick="location.href='${ path }/member/login'" class="btn btn-outline-secondary">로그인</button>             
					</c:if>
					<c:if test="${ not empty loginMember }">
						${ loginMember.name }님 안녕하세요.
						<br><br>
						<button onclick="location.href='${ path }/member/myPage'" class="btn btn-outline-secondary">내 정보</button>
						<button onclick="location.replace('${ path }/logout')" class="btn btn-outline-secondary">로그아웃</button>
					</c:if>
            	</div>
           </div>
    </nav>
...
<script>
	$(document).ready(() => {
		...
		$('#btntest').on('click', () => {
			location.replace("${ path }/views/member/test.jsp");	
		});
