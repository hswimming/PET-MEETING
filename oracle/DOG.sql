...
COMMENT ON COLUMN DOG_INFO.D_ORI_IMG IS '원래 사진 이름';
COMMENT ON COLUMN DOG_INFO.D_RE_IMG IS '새로운 사진 이름';
...
SELECT D_NUM, D_ORI_IMG, D_RE_IMG, D_NAME, D_KIND, D_SIZE, D_GENDER, NEUTERED, VACCINE, MEM_CODE
FROM DOG_INFO
WHERE MEM_CODE = 1;
...
CREATE TABLE MEMBER (
    ...
    MEM_PWD VARCHAR2(150) CONSTRAINT MEM_PWD_NN NOT NULL,
...
CREATE TABLE ADMIN (
	...
	ADM_PWD VARCHAR2(500) NOT NULL,
    ADM_ROLE VARCHAR2(10) NOT NULL, 
	CONSTRAINT ADMIN_ADM_ID_UQ UNIQUE(ADM_ID)
);

COMMENT ON COLUMN ADMIN.ADM_ID IS '관리자 아이디'; 
COMMENT ON COLUMN ADMIN.ADM_PWD IS '관리자 비밀번호'; 
COMMENT ON COLUMN ADMIN.ADM_ROLE IS '관리자 역할'; 
...
SELECT * FROM ADMIN WHERE ADM_ID='admin';

CREATE TABLE RECO_WALK_LIST(
    LIST_NO NUMBER,
    LIST_IMG_ORI VARCHAR2(500) NOT NULL,
    LIST_IMG_RE VARCHAR2(500) NOT NULL,
    LIST_CONTENT VARCHAR2(1000) NOT NULL,
    CONSTRAINT RECO_WALK_LIST_LIST_NO_PK PRIMARY KEY("LIST_NO")
    );
